...
def make_arg_parser():
    parser = argparse.ArgumentParser(description="""
        A script to extract rows specifically for Fidelity only, for a given customer.
        
        Examples:
            python extract_records_from_consolidated_Fidelity.py -m INPUT_FILE*.txt -c "CODE"
    """)
...
def get_code_pattern(code: str, kind: str) -> re.Pattern:
    """Each customer using the Fidelity feed has their data "bounded" by a "customer header" (CH) row and a "customer trailer" (CT) row."""
    p = {
        'start': f'^CH.*{code}',
        'end': f'^CT.*{code}'
    }
...
def parse_records(records: list) -> dict:
    ...
    for i, v in enumerate(records):
        r[i] = {
            'account_provider_name': 'Fidelity',
